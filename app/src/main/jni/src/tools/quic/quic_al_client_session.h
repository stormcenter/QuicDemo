...
#ifndef NET_TOOLS_QUIC_QUIC_AL_CLIENT_SESSION_H_
#define NET_TOOLS_QUIC_QUIC_AL_CLIENT_SESSION_H_
...
class QuicServerId;
 class QuicALClientSession : public QuicSession ,
     public QuicCryptoClientStream::ProofHandler {
 public:
  // Takes ownership of |connection|. Caller retains ownership of
  // |promised_by_url|.
  QuicALClientSession(const QuicConfig& config,
                        QuicConnection* connection,
                        const QuicServerId& server_id,
                        QuicCryptoClientConfig* crypto_config);
  ~QuicALClientSession() override;
  ...
  QuicALClientStream* CreateOutgoingDynamicStream() override;
  QuicCryptoStream* GetMutableCryptoStream() override;
  const QuicCryptoStream* GetCryptoStream() const override;

  // Create the crypto stream. Called by Initialize().
  virtual std::unique_ptr<QuicCryptoClientStream> CreateQuicCryptoStream();
  ...
  virtual std::unique_ptr<QuicALClientStream> CreateClientStream();
  ...
  bool IsAuthorized(const QuicString& authority) ;

  // QuicSpdyClientSessionBase methods:
  //void OnProofValid(const QuicCryptoClientConfig::CachedState& cached) ;
  //void OnProofVerifyDetailsAvailable(
 ...
 private:
  ...
  DISALLOW_COPY_AND_ASSIGN(QuicALClientSession);
};
